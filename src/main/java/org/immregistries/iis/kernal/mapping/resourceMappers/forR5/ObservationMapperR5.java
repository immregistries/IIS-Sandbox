...
public class ObservationMapperR5 implements ObservationMapper<Observation>, IR5Mapper<ObservationMaster, Observation> {
	@Autowired
	...
	public ObservationReported localObjectReportedWithMaster(Observation observation) {
		...
		// new SearchParameterMap(Observation.SP_IDENTIFIER, new
		// TokenParam().setSystem(observationReported.getIdentifierTable()).setValue(observationReported.getIdentifierCode()))
		// // TODO remove
	...
	public Observation fhirResource(ObservationMaster om) {
		 ...
		 */
		DataType value = getObservationReportedValue(om);
		o.setValue(value);
		/*
		 ...
		 * OBX-11
		 */
		Enumerations.ObservationStatus observationStatus;
		switch (om.getResultStatus()) {
			case "A": {
				observationStatus = Enumerations.ObservationStatus.AMENDED;
				break;
			}
			case "C": {
				observationStatus = Enumerations.ObservationStatus.CORRECTED;
				break;
			}
			case "D": {
				observationStatus = Enumerations.ObservationStatus.ENTEREDINERROR;
				break;
			}
			case "F": {
				observationStatus = Enumerations.ObservationStatus.FINAL;
				break;
			}
			case "P": {
				observationStatus = Enumerations.ObservationStatus.PRELIMINARY;
				break;
			}
			case "X": {
				Extension extension = o.addExtension()
						.setUrl("http://hl7.org/fhir/StructureDefinition/alternate-codes");
				CodeableConcept codeableConcept = new CodeableConcept();
				codeableConcept.addCoding().setSystem("http://terminology.hl7.org/CodeSystem/v2-0085").setCode("X");
				extension.setValue(codeableConcept);
				observationStatus = Enumerations.ObservationStatus.CANCELLED;
				break;
			}
			case "N": {
				o.setDataAbsentReason(new CodeableConcept(new Coding().setCode("not-asked")
						.setSystem("http://terminology.hl7.org/CodeSystem/data-absent-reason")));
				observationStatus = Enumerations.ObservationStatus.UNKNOWN;
				break;
			}
			case "B":
			case "I":
			...
			case "W":
			default: {
				observationStatus = Enumerations.ObservationStatus.NULL;
				break;
			}
		}
	...
	private static Observation.ObservationComponentComponent fhirObservationComponent(ObservationMaster component) {
		...
		if (component.getValueType() != null) {
			observationComponent.addExtension().setUrl(OBS_TYPE_OBX_2)
					.setValue(new StringType(component.getValueType()));
		}
		/*
		 * OBX-3
		 */
	...
	public ObservationReported localObjectReported(Observation o) {
		 ...
		 * Vaccination Part of
		 */
		Reference vaccinationReference = o.getPartOf().stream()
				.filter(ref -> ref.getReference().startsWith("Immunization/")).findFirst().orElse(null);
		...
		if (o.hasValueCodeableConcept() && o.getValueCodeableConcept().hasCoding()) {
			...
			// observationReported.setUnitsCode(quantity.getUnit()); // TODO get from
			// codemap ?
	...
	private static @NotNull ObservationMaster fromFhirComponent(
		...
		if (observationComponent.hasValueCodeableConcept()
				&& observationComponent.getValueCodeableConcept().hasCoding()) {
		...
		switch (om.getValueType()) {
			...
			case "SN": {
				if ("<>".equals(om.getValueCode())) { // TODO complete
					...
					numerator.setComparator(Enumerations.QuantityComparator.valueOf(om.getValueCode()));
