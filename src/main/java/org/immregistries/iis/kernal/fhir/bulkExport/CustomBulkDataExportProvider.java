...
//
	...
	public void export(
		...
		// JPA export provider
		if (hasAsyncHeader(theRequestDetails, ProviderConstants.OPERATION_EXPORT, )) {
	...
	private boolean shouldUseCache(ServletRequestDetails theRequestDetails) {
		...
		return myJpaStorageSettings.getEnableBulkExportJobReuse() && !cacheControlDirective.isNoCache();
	}
...
//			bulkQueryGroupProviderR5.groupInstanceSynchExport(new IdType(theIdParam.getValue()), theOutputFormat, null, null, theLastUpdated, null, null, null, theTypes, null, theRequestDetails);
	 ...
	 * $export-poll-status
	 */
	@Operation(name = ProviderConstants.OPERATION_EXPORT_POLL_STATUS, manualResponse = true, idempotent = true, deleteEnabled = true)
	public void exportPollStatus(
	...
	public void writePollingLocationToResponseHeaders(ServletRequestDetails theRequestDetails, JobInfo theOutcome) {
		...
		String pollLocation = serverBase + "/" + ProviderConstants.OPERATION_EXPORT_POLL_STATUS + "?" + JpaConstants.PARAM_EXPORT_POLL_STATUS_JOB_ID + "=" + theOutcome.getJobMetadataId();
...
//			throw new InvalidRequestException(Msg.code(513) + "Must request async or sync processing for " + theOperationName);
	...
	@VisibleForTesting
	public void setJpaStorageSettings(JpaStorageSettings theJpaStorageSettings) {
