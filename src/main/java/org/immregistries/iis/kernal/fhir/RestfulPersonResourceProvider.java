...
public class RestfulPersonResourceProvider implements IResourceProvider {
    ...
    private Person getPersonById(String idPart, Session dataSession, OrgAccess orgAccess) {
        ...
        Query query = dataSession.createQuery(
                "from PatientReported where patientReportedExternalLink = ?");
        ...
        Query queryLink = dataSession.createQuery(
                "from PatientLink where patientMaster.id = ?");
        ...
        if(patientLinkList.size()>0){
            for(PatientLink link : patientLinkList){
                ...
                if(assuranceLevel==1){
                    person.addLink(personLinkComponent.setTarget(reference).setAssurance(Person.IdentityAssuranceLevel.LEVEL2));

                }else {
                    person.addLink(personLinkComponent.setTarget(reference).setAssurance(Person.IdentityAssuranceLevel.LEVEL3));
                }
            }
    ...
    @Create
    public MethodOutcome createPatient(RequestDetails theRequestDetails, @ResourceParam Person thePerson) {
    ...
    @Update
    public MethodOutcome updatePerson(RequestDetails theRequestDetails, @IdParam IdType theId , @ResourceParam Person thePerson) {
        return new MethodOutcome();
    }
