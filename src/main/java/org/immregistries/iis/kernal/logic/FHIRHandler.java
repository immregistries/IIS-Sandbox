...
public class FHIRHandler extends IncomingMessageHandler {
  ...
  public VaccinationReported FHIR_EventVaccinationReported(OrgAccess orgAccess, Patient patient,
      PatientReported patientReported, Immunization immunization) throws Exception {
    ...
    ImmunizationHandler.vaccinationReportedFromFhirImmunization(vaccinationReported, immunization);
    {
      ...
      if (vaccinationReportedList.size() > 0) { // if external link found
        ...
        vaccinationMaster =
            ImmunizationHandler.findMatch(dataSession, patientReported, immunization);
      }
    }
