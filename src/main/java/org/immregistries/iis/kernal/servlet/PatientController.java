...
public class PatientController {
	...
	private void singlePatientInformationPrintAll(PrintWriter out, IBaseResource patientSelected,
			IGenericClient fhirClient, Tenant tenant, Session dataSession, HttpServletRequest req) {
		...
		printMessages(out, dataSession, patientMasterSelected);
	...
	private void printPatientRecommendationsAndSubscriptions(PrintWriter out, IBaseResource patientSelected,
			PatientMaster patientMasterSelected, IGenericClient fhirClient) {
	...
	private void printRelatedPatients(PrintWriter out, PatientMaster patientMasterSelected, boolean isGolden) {
		...
		if (isGolden) {
			relatedPatients = fhirRequester
					.searchPatientReportedFromGoldenIdWithMdmLinks(patientMasterSelected.getPatientId());
		} else {
			PatientMaster goldenRecord = fhirRequester
					.readPatientMasterWithMdmLink(patientMasterSelected.getPatientId());
	...
	private void printMessages(PrintWriter out, Session dataSession, PatientMaster patientMasterSelected) {
		out.println("<div class=\"w3-container\">");
	...
	private void printSearchForm(PrintWriter out, String patientNameLast, String patientNameFirst,
			String externalLink) {
	...
	private List<PatientMaster> searchPatient(HttpServletRequest req) {
	...
	private void searchOrPrintAll(HttpServletRequest req, PrintWriter out, Tenant tenant) {
		List<PatientMaster> patientMasterList = searchPatient(req);

		String patientNameLast = StringUtils.defaultString(req.getParameter(PARAM_PATIENT_NAME_LAST));
		String patientNameFirst = StringUtils.defaultString(req.getParameter(PARAM_PATIENT_NAME_FIRST));
		String externalLink = StringUtils.defaultString(req.getParameter(PARAM_PATIENT_REPORTED_EXTERNAL_LINK));

		printSearchForm(out, patientNameLast, patientNameFirst, externalLink);
		...
		printFhirShortcutsAllPatients(out, tenant);
	}

	private void printFhirShortcutsAllPatients(PrintWriter out, Tenant tenant) {
		out.println("<div class=\"w3-container\">");
