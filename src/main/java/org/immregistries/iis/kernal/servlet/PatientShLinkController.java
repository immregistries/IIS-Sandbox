...
//				shLinkManifest = shlUtilService.saveManifest(shLinkManifest);
	...
	private String getQrcode(HttpServletRequest req, IBaseResource patientSelected) {
		ShLinkPayload shLinkPayload = getPatientShLinkPayload(req, patientSelected);

		return shlUtilService.qrCode(shLinkPayload);
	}

	private static @NotNull ShLinkPayload getPatientShLinkPayload(HttpServletRequest req, IBaseResource patientSelected) {
	...
	private static @NotNull String getManifestUrl(HttpServletRequest req, IBaseResource patientSelected) {
		return StringUtils.substringBeforeLast(req.getServletPath(), SHLINK_QR_CODE_PATH_SUFFIX) + MANIFEST_PATH_SUFFIX + "?id=" + patientSelected.getIdElement().getIdPart();
	}
