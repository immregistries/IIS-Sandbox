...
public class HomeController {
	...
	public static void doHeader(PrintWriter out, String title, Tenant tenant) {
		...
		if (authentication.isAuthenticated() && !(authentication instanceof AnonymousAuthenticationToken)) {
			...
			if (tenant != null) {
				...
				out.println("<a href=\"" + RepositoryClientFactory.fhirServerBasePath(tenant)
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			doHeader(out, "IIS Sandbox - Home", CurrentTenantUtil.getTenant());
			...
			if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R5)) {
				out.println(
						"      <li><a href=\"fhir\">FHIR Server</a>: FHIR Restful server, partitioned for each tenants</li>");
			} else if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R4)) {
				out.println(
						"      <li><a href=\"fhir\">FHIR Server</a>: FHIR Restful server, partitioned for each tenants</li>");
			}
			...
			out.println("    <ul class=\"w3-ul w3-hoverable\">");
			out.println(
					"      <li><h4>HAPIFHIR Server Backend:</h4> This sandbox uses HAPIFHIR JPA Server framework as an end layer to store records, "
							+
			...
			out.println(
					"      <li><h4>Consolidated (Golden) record:</h4>Golden records are generated by MDM system as the reference records to match with and merge information in");
			out.println(
					"      <li><h4>Bulk data exchange server:</h4>Bulk data <a href='https://build.fhir.org/ig/HL7/bulk-data/export.html'>export</a> implemented, "
							+
