...
//		,FhirMessagingController.FHIR_MESSAGING, "Conversion messaging"
	...
	public static void doHeader(PrintWriter out, String title, Tenant tenant) {
		...
		for (Map.Entry<String, String> header : HEADER_MAP.entrySet()) {
			out.println("<a href=\"" + ServletHelper.tenantifyUrl(tenant, header.getKey()) + "\" class=\"w3-bar-item w3-button\">" + header.getValue() + "</a>");
		}
//		out.println("<a href=\"subscription\" class=\"w3-bar-item w3-button\">Subscriptions</a>");
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R5)) {
				out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "subscription") + "\">Subscriptions</a>: Visualize and manually trigger FHIR subscriptions</li>");
				out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, FhirMessagingController.FHIR_MESSAGING_PATH_KEY) + "\">Conversion messaging (Unavailable in R5 mode)</a>: Experimental endpoint for FHIR messaging with messages converted from HL7v2</li>");
			} else if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R4)) {
				out.println("      <li><a>Subscriptions</a>: (Unavailable in R4 mode) Visualize and manually trigger FHIR subscriptions</li>");
				out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, V2ToFhirController.V2_TO_FHIR_PATH_KEY) + "\">V2ToFhir</a>: V2 to Fhir conversion using v2ToFhir dependency</li>");
				out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, FhirMessagingController.FHIR_MESSAGING_PATH_KEY) + "\">Conversion messaging </a>: Experimental endpoint for FHIR messaging with messages converted from HL7v2</li>");

			}
			out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "soap") + "\">CDC WSDL</a>: HL7 realtime interfacing using CDC WSDL</li>");
			if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R5)) {
			...
			out.println("    <ul class=\"w3-ul w3-hoverable\">");
			out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "lab") + "\">Convert ORU to VXU</a>: Convert an ORU lab message to a VXU. </li>");
			out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "queryConverter") + "\">Convert VXU to QBP</a>: Convert an VXU immunization message into an immunization query. </li>");
			if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R5)) {
				out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "covid") + "\">COVID-19 Reporting</a>: Export data to demonstrate COVID-19 reporting </li>");
			} else if (fhirContext.getVersion().getVersion().equals(FhirVersionEnum.R4)) {
				out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "covid") + "\">COVID-19 Reporting</a>:(Unavailable in R4 mode)  Export data to demonstrate COVID-19 reporting </li>");
			}
			out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "VXUDownloadForm") + "\">COVID-19 Reporting (HL7)</a>: Download data in HL7 format demonstrate COVID-19 reporting </li>");
			out.println("      <li><a href=\"" + ServletHelper.tenantifyUrl(tenant, "covidGenerate") + "\">COVID-19 HL7 Generator</a>: Generate HL7 Messages</li>");
